...
Deno.serve(async (req) => {
  try {
    ...
    const { error: facErr } = await supabase
      .from('factions')
      .upsert(factions, { onConflict: 'code' });
    if (facErr) throw new Error('Fraktionen: ' + facErr.message);

    const { data: facRows, error: facReadErr } = await supabase
      .from('factions')
      .select('id, code');
    if (facReadErr) throw new Error('Fraktionen lesen: ' + facReadErr.message);
    const facId = Object.fromEntries(facRows.map((f) => [f.code, f.id]));
    ...
    const { error: goodsErr } = await supabase
      .from('goods')
      .upsert(goods, { onConflict: 'code' });
    if (goodsErr) throw new Error('Waren: ' + goodsErr.message);

    const { data: goodRows, error: goodReadErr } = await supabase
      .from('goods')
    ...
    // actors hat keinen Unique-Constraint auf display_name -> nur fehlende einfügen.
    ...
    //    Landmassen in src/lib/mapGeography.js).
    // -------------------------------------------------------------------------
    const ports = [
    ...
    const { data: portRows, error: portReadErr } = await supabase
      .from('ports')
      .select('id, code');
